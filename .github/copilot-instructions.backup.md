# ARSAFE_URP – Agent Instructions

_Last updated: October 19, 2025 (Documentation consolidation)_
...
4. **Sync with CONTEXT_MEMORY** - Major architectural changes belong in both files
...
4. ✅ **UPDATE CONTEXT_MEMORY.md** - Document ALL significant changes immediately
...
- Forget to update CONTEXT_MEMORY.md → STOP. Update it NOW.
...
1. **📝 Documentation First**
   - `.github/CONTEXT_MEMORY.md` is the **single source of truth** for live system state
   - **NEW STRUCTURE:** Quick Reference → Current Architecture → Recent Changes (30 days) → Troubleshooting → Historical Archive
   - Update Section 3 (Recent Changes) after every significant change with timestamp
   - Archive entries older than 30 days to Section 6 (Historical Changes Archive)
...
2. **🔍 Research Before Code**
   ...
   - Document findings in comments or `CONTEXT_MEMORY.md`
...
5. **🐛 Debug & Documentation Updates**
   - **ALWAYS update after making changes:**
     ...
     - Update `.github/CONTEXT_MEMORY.md` with new entries
...
├─ COMPREHENSIVE_CODE_REVIEW_AND_FIXES.md   (Code review, fixes, performance)
├─ DISASTER_SYSTEMS_COMPLETE_GUIDE.md       (Earthquake + Flood complete setup)
├─ GPS_EXTERNAL_LOCATION_PRIOR_INTEGRATION.md (Future GPS feature)
└─ MULTIAREA_COMPARISON.md                   (Vuforia base vs ARSafe)

.github/
├─ CONTEXT_MEMORY.md                         (Live system state - UPDATE ALWAYS)
└─ copilot-instructions.md                   (This file - Agent guidelines)
```

**When to Update Which File:**
- **CONTEXT_MEMORY.md** → ALL significant code changes, new features, bug fixes (timestamp entries)
- **COMPREHENSIVE_CODE_REVIEW_AND_FIXES.md** → Performance optimizations, bug fixes, code quality improvements
- **DISASTER_SYSTEMS_COMPLETE_GUIDE.md** → Earthquake/Flood system changes, new disaster features
- **GPS_EXTERNAL_LOCATION_PRIOR_INTEGRATION.md** → GPS feature planning/implementation only
...
1. `.github/CONTEXT_MEMORY.md` (Live system state - update after EVERY significant change)
2. Consolidated guides:
   ...
   - `GPS_EXTERNAL_LOCATION_PRIOR_INTEGRATION.md` (future GPS)
   - `MULTIAREA_COMPARISON.md` (architecture comparison)
...
**CRITICAL: This is an AR mobile application. ALL UI must be designed for phone screens.**
- **Minimum font sizes:** Body text ≥22px, headings ≥26px, titles ≥32px
- **Touch targets:** Buttons and interactive elements ≥60px × 60px
- **Spacing:** Generous padding (24px+) and margins (16px+) for readability
- **Test assumption:** If text/buttons look good on desktop, they're too small for mobile
- When in doubt, go 40-60% larger than typical web CSS sizes
...
5. **Capture Findings** in code comments or `CONTEXT_MEMORY.md`
...
5. **Post-Implementation Updates (MANDATORY - DO NOT SKIP)**
   - ✅ **Update `CONTEXT_MEMORY.md`** with timestamped entry documenting the change
...
- Project-specific code patterns already documented in `CONTEXT_MEMORY.md`
...
- **Reference** `CONTEXT_MEMORY.md` for up-to-date keyword lists, priority boosts (+200 connected rooms, +50 approaching), and debugging tips.
...
- See `Assets/ARSafe_ModularSystem/Documentation/EARTHQUAKE_SETUP.md` for complete integration guide.
...
5. **Update documentation**: add notes to `CONTEXT_MEMORY.md`, refresh public API comments, and extend `ARDebugLogger.filterKeywords[]` for new log phrases.
...
- Consult `Assets/ARSafe_ModularSystem/Documentation/QUICK_REFERENCE.md` + `ARCHITECTURE.md` before adding parameters.
...
### Critical Files
- `.github/CONTEXT_MEMORY.md` - System state and recent changes
...
4. **System Changes** → Map dependencies → Update all related scripts → Add debug keywords → Update CONTEXT_MEMORY
...
4. ✅ **Review CONTEXT_MEMORY** - Check for existing patterns and recent changes
...
2. ✅ **Update CONTEXT_MEMORY** - Add new entry with timestamp
...
4. ✅ **Documentation** - Update CONTEXT_MEMORY and README
